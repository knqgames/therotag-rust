...
function PLUGIN:Init()
	...
	self.Config.Messages.Transfer_Error = self.Config.Messages.Transfer_Error or "You can not transfer money yourself!"
	...
	command.AddConsoleCommand( "bnc.c", self.Object, "CC_Bnc" )

	if cmds.BB ~= "" then command.AddChatCommand(cmds.BB, self.Object, "C_BB") end
	if cmds.SetBB ~= "" then command.AddChatCommand(cmds.SetBB, self.Object, "C_SetBB") end
	if cmds.TB ~= "" then command.AddChatCommand(cmds.TB, self.Object, "C_TB") end
	if cmds.DB ~= "" then command.AddChatCommand(cmds.DB, self.Object, "C_DB") end
	if cmds.WB ~= "" then command.AddChatCommand(cmds.WB, self.Object, "C_WB") end	
end	
	...
	if args.Length > 1 then
		...
		if money then
			if money > 0 then
				local target   = global.BasePlayer.Find(args[0])
				if target then
					local DepositWithFee = money + math.floor(money * (self.DepositFee / 100))
					local DepositFee = DepositWithFee - money	
					...
					if(isLimit < self.Limit) then
						if (money <= self.Limit) then
							if (DepositWithFee <= moneyCheck) then
								if (tonumber(moneyCheck) > money) then
									EcoAPI:GetUserDataFromPlayer(player):Withdraw(DepositWithFee)
